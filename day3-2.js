...
const path1 = input1.split(',')
const path2 = input2.split(',')

const computePositions = path => {
  ...
  path.forEach(move => {
    ...
    for (let i = 1; i <= distance; i++) {
      ...
      steps += 1
      positions.set(`${current[0]}#${current[1]}`, steps)
    }
  })
  return positions
}

const positions1 = computePositions(path1)
const positions2 = computePositions(path2)

const intersections = Array.from(new Set([...positions1.keys()].filter(x => positions2.has(x))))

function getCombinedSteps(p) {
  const point1 = positions1.get(`${p[0]}#${p[1]}`)
  const point2 = positions2.get(`${p[0]}#${p[1]}`)
...
const sorted = intersections.sort((a, b) => {
  ...
  return getCombinedSteps(p1) - getCombinedSteps(p2)
})

const best = sorted[0].split('#').map(Number)
console.log(best[0], best[1], Math.abs(best[0]) + Math.abs(best[1]))

console.log(getCombinedSteps(best))
