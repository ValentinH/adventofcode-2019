const computePositions = path => {
  ...
  path.forEach(move => {
    ...
    for (let i = 1; i <= distance; i++) {
      ...
      positions.add(`${current[0]}#${current[1]}`)
    }
  })
  return positions
}
...
const intersectionSet = new Set([...positions1].filter(x => positions2.has(x)))
const intersections = Array.from(intersectionSet)

const sorted = intersections.sort((a, b) => {
  const p1 = a.split('#').map(Number)
  const p2 = b.split('#').map(Number)
  return Math.abs(p1[0]) + Math.abs(p1[1]) - (Math.abs(p2[0]) + Math.abs(p2[1]))
})
