...
function getParameter(input, value, mode, relativeBase) {
  if (mode === 1) return value
  if (mode === 2) return input[relativeBase + value]
  return input[value] || 0
}
function getDestination(value, mode, relativeBase) {
  if (mode === 2) return relativeBase + value
  return value
}

const computer = (program, inputValues, position = 0, relativeBase = 0) => {
  while (1) {
    ...
    switch (opcode.opcode) {
      ...
      case 1: {
        const f1 = getParameter(program, program[position + 1], opcode.mode1, relativeBase)
        const f2 = getParameter(program, program[position + 2], opcode.mode2, relativeBase)
        const destination = getDestination(program[position + 3], opcode.mode3, relativeBase)
        program[destination] = f1 + f2
        DEBUG && console.log('ADD', destination, f1 + f2)
        position += 4
        break
      }
      case 2: {
        ...
        const destination = getDestination(program[position + 3], opcode.mode3, relativeBase)
        program[destination] = f1 * f2
        DEBUG && console.log('MUL', destination, f1 * f2)
        position += 4
        break
      }
      case 3: {
        const inputValue = inputValues.shift()
        const destination = getDestination(program[position + 1], opcode.mode1, relativeBase)
        program[destination] = inputValue

        DEBUG && console.log('INPUT', destination, inputValue)
        position += 2
        break
      }
      case 4: {
      ...
      case 7: {
        ...
        const f2 = getParameter(program, program[position + 2], opcode.mode2, relativeBase)
        const destination = getDestination(program[position + 3], opcode.mode3, relativeBase)
        program[destination] = f1 < f2 ? 1 : 0
        DEBUG && console.log('LOWER THAN', destination, f1 < f2 ? 1 : 0)
        position += 4
        break
      }
      case 8: {
        ...
        const destination = getDestination(program[position + 3], opcode.mode3, relativeBase)
        program[destination] = f1 === f2 ? 1 : 0
        DEBUG && console.log('EQUAL', destination, f1 === f2 ? 1 : 0)
        position += 4
        break
      }
      case 9: {
        const f1 = getParameter(program, program[position + 1], opcode.mode1, relativeBase)
        DEBUG && console.log('ADJUST RELATIVE BASE', relativeBase, relativeBase + f1)
        relativeBase += f1
        position += 2
        break
      }
...
function run(program, input) {
  ...
  while (1) {
    ...
    result = computer(nextProgram, nextInput, nextPosition, relativeBase)
